...
ptri _memsize( void* p )
{
	...
	for(cur = (struct alloc*)(((ptri)head->next << 2) | EWRAM); ((cur->next << 2) | EWRAM) != head; cur = (struct alloc*)(((ptri)cur->next << 2) | EWRAM))
	{
...

	{
		...
		for( ;; )
		{
			if( !( pos->flags & FLAGS_INUSE_MASK ) )
			{
				...
				if( foundsz - sz < ( sizeof( struct alloc ) << 1 ) )
				{
					/* it isn’t much bigger than requested size
					 * just use it */
					pos->flags |= FLAGS_INUSE_MASK;
				}
				else
				{
					/* block is much bigger than requested size
					 * split off the rest */
					struct alloc* split;
					struct alloc* temp;
					...
					split = (struct alloc*)( pos + sizeof( struct alloc ) + sz );
					pos->flags |= FLAGS_INUSE_MASK;
					pos->wordsz = sz >> 2;

					split->flags  = MAGICNUM;
					split->wordsz = foundsz >> 2;
					split->prev   = ( (ptri)pos >> 2 ) & 0xFFFF;
					split->next   = ( (ptri)pos->next >> 2 ) & 0xFFFF;

					pos->next = ( (ptri)split >> 2 ) & 0xFFFF;

					temp = (struct alloc*)( ( split->next << 2 ) | EWRAM );

					if( temp != head )
					{
						temp->prev = ( (ptri)split >> 2 ) & 0xFFFF;
					}

					return (u8*)pos + sizeof( struct alloc );
				}
			}

			if( (struct alloc*)( ( pos->next << 2 ) | EWRAM ) == head )
			{
				return NULL;
			}
...
void free( void* ptr )
{
	...
	{
		struct alloc* head = (struct alloc*)EWRAM;
		struct alloc* blk = (struct alloc*)( (u8*)ptr - sizeof( struct alloc ) );
		struct alloc* next = (struct alloc*)( ( blk->next << 2 ) | EWRAM );

		blk->flags &= ~FLAGS_INUSE_MASK;

		/* if the freed block is not last, merge it with the next block if it is
		 * also not in use */

		if( next != head && !( next->flags & FLAGS_INUSE_MASK ) )
		{
			blk->wordsz += ( sizeof( struct alloc ) >> 2 ) + next->wordsz;
		...
		/* if the freed block is not first, merge it with the previous block if
		 * it is also not in use */
		if( blk != head )
		{
			struct alloc* prev = (struct alloc*)( ( blk->prev << 2 ) | EWRAM );
			struct alloc* next = (struct alloc*)( ( blk->next << 2 ) | EWRAM );
			...
			if( !( prev->flags & FLAGS_INUSE_MASK ) )
			{
				...
				prev->wordsz += ( sizeof( struct alloc ) >> 2 ) + blk->wordsz;
			}
		}
	}
}
