...
enum { N = 624, M = 397, LOWER_MASK = 0x7FFFFFFF };
...
static void set_seed(struct mt_prng *prng, u32 seed) {
  ...
  for (prng->mti = 1; prng->mti < N; ++prng->mti) {
    prng->mt[prng->mti] = 1812433253u * (prng->mt[prng->mti - 1] ^
...
static void set_seed_arr(struct mt_prng *prng, u32 *seed, ptri seed_sz) {
  ...
  for (i = j = 0, k = N > seed_sz ? N : seed_sz; k != 0; --k) {
    prng->mt[i] = (prng->mt[i] ^
                   ((prng->mt[i - 1] ^ (prng->mt[i - 1] >> 30)) * 1664525u)) +
                  seed[j] + j;
    prng->mt[i] &= 0xFFFFFFFFu; /* for 64-bit and larger machines */
  ...
  for (k = N - 1; k != 0; --k) {
    prng->mt[i] = (prng->mt[i] ^ ((prng->mt[i - 1] ^ (prng->mt[i - 1] >> 30)) *
                                  1566083941u)) -
                  i;
    prng->mt[i] &= 0xFFFFFFFFu; /* for 64-bit and larger machines */
...
u32 mt_random_u32(struct mt_prng *prng) { return (u32)mt_random_s32(prng); }

s64 mt_random_s64(struct mt_prng *prng) {
  return (s64)mt_random_s32(prng) | ((s64)mt_random_s32(prng) << 32);
}

u64 mt_random_u64(struct mt_prng *prng) { return (u64)mt_random_s64(prng); }
