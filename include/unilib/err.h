...
#else /* !defined(NDEBUG) */
#define ASSERT(cnd)                                                            \
  ((cnd) ? (void)0 : uni_assert_fail(#cnd, __FILE__, __LINE__, __func__))
...
#define IMPL_ASSERT_SDL(v, r, file, line)                                      \
  do {                                                                         \
    if (v) {                                                                   \
      ...
      fprintf(stderr, "SDL error @ %s L%u: %s\n", file, line, SDL_GetError()); \
      if (r) {                                                                 \
        return r;                                                              \
      }                                                                        \
    }                                                                          \
...
#define IMPL_ASSERT_TTF(v, r, file, line)                                      \
  do {                                                                         \
    if (v) {                                                                   \
      ...
      fprintf(stderr, "SDL_TTF error @ %s L%u: %s\n", file, line,              \
              TTF_GetError());                                                 \
      if (r) {                                                                 \
        return r;                                                              \
      }                                                                        \
...
#define IMPL_ASSERT_LODEPNG(v, r, file, line)                                  \
  do {                                                                         \
    if (v) {                                                                   \
      ...
      fprintf(stderr, "LodePNG error @ %s L%u: %s\n", file, line,              \
...
#define IMPL_ASSERT_NOP(x, o, file, line)                                      \
  do {                                                                         \
    if (x) {                                                                   \
      ...
      fprintf(stderr, "!!! Assert failed @ %s L%u: %s\n", file, line, #x);     \
      o.is = 0;                                                                \
      return (o);                                                              \
    }                                                                          \
  } while (0)
